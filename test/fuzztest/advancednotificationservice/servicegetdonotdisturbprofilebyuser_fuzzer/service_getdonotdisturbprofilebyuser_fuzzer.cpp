 ...
 */
#include "service_getdonotdisturbprofilebyuser_fuzzer.h"
...
namespace Notification {
    bool DoSomethingInterestingWithMyAPI(FuzzedDataProvider *fuzzData)
    {
        ...
        int64_t intData = fuzzData->ConsumeIntegral<uint64_t>();
        int32_t userId = fuzzData->ConsumeIntegralInRange<int32_t>(0, 100);
        sptr<NotificationDoNotDisturbProfile> profile = ObjectBuilder<NotificationDoNotDisturbProfile>::Build(fuzzData);
        service->GetDoNotDisturbProfile(intData, profile, userId);
