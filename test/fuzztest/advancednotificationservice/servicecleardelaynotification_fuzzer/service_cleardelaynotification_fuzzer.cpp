 ...
 */
#include "service_cleardelaynotification_fuzzer.h"
...
constexpr int32_t MAX_USER_ID = 100;
    ...
    bool DoSomethingInterestingWithMyAPI(FuzzedDataProvider *fuzzData)
    {
        ...
        auto triggerKeysSize = fuzzData->ConsumeIntegralInRange<int32_t>(0, MAX_VECTOR_SIZE);
        std::vector<std::string> triggerKeys;
        for (int i = 0; i < triggerKeysSize; ++i) {
            triggerKeys.push_back(fuzzData->ConsumeRandomLengthString());
        }
