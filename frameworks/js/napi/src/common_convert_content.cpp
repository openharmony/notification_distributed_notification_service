    ...
    const napi_value &contentResult, std::shared_ptr<OHOS::Notification::NotificationPictureContent> &pictureContent)
{
    ...
    if (!hasProperty) {
        ANS_LOGE("Property pic expected.");
        return nullptr;
    }
    ...
    std::shared_ptr<OHOS::Notification::NotificationConversationalContent> &conversationalContent)
{
    ...
    if (!hasProperty) {
        ANS_LOGE("Property msgs expected.");
        return nullptr;
    }
    ...
    if (!isArray) {
        ANS_LOGE("Property msgs is expected to be an array.");
        return nullptr;
    }
    uint32_t length = 0;
