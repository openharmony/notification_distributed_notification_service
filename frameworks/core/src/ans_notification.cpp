...
ErrCode AnsNotification::RequestEnableNotification(std::string &deviceId, const sptr<IRemoteObject> &callbackInfo)
{
    ANS_LOGD("enter");
    if (!GetAnsManagerProxy()) {
        ANS_LOGE("GetAnsManagerProxy fail.");
    ...
    return ansManagerProxy_->RequestEnableNotification(deviceId, callbackInfo);
}
