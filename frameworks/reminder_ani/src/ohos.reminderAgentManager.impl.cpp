...
static bool UnWarpNotificationSlot(uintptr_t slot, Notification::NotificationConstant::SlotType& outSlot)
{
    ...
    outSlot = Notification::NotificationConstant::SlotType::OTHER;
...
double PublishReminderSync(::ohos::reminderAgentManager::manager::ParamReminder const& reminderReq)
{
    ...
    int32_t ret = Notification::ReminderHelper::PublishReminder(*reminder, reminderId);
    if (ret != ERR_OK) {
        ::taihe::set_business_error(ret, ReminderAgentManagerNapi::Common::getErrCodeMsg(ret));
        return -1;
    }
...
void CancelReminderSync(double reminderId)
{
    ...
    int32_t ret = Notification::ReminderHelper::CancelReminder(id);
    if (ret != ERR_OK) {
        ::taihe::set_business_error(ret, ReminderAgentManagerNapi::Common::getErrCodeMsg(ret));
    }
}
...
::taihe::array<::ohos::reminderAgentManager::manager::ParamReminder> GetValidRemindersSync()
{
    ...
    int32_t ret = Notification::ReminderHelper::GetValidReminders(reminders);
...
{
    int32_t ret = Notification::ReminderHelper::CancelAllReminders();
    if (ret != ERR_OK) {
        ::taihe::set_business_error(ret, ReminderAgentManagerNapi::Common::getErrCodeMsg(ret));
    }
}
...
{
    Notification::NotificationConstant::SlotType notificationSlot;
    int32_t ret = ReminderAgentManagerNapi::Common::ERR_REMINDER_INVALID_PARAM;
    if (!UnWarpNotificationSlot(slot, notificationSlot)) {
        ::taihe::set_business_error(ret, ReminderAgentManagerNapi::Common::getErrCodeMsg(ret));
        return;
    }
    ret = Notification::NotificationHelper::AddSlotByType(notificationSlot);
    if (ret != ERR_OK) {
        ::taihe::set_business_error(ret, ReminderAgentManagerNapi::Common::getErrCodeMsg(ret));
    }
}
...
{
    int32_t ret = ReminderAgentManagerNapi::Common::ERR_REMINDER_INVALID_PARAM;
    Notification::NotificationConstant::SlotType slot;
    if (!ReminderAgentManagerNapi::Common::UnWarpSlotType(slotType, slot)) {
        ::taihe::set_business_error(ret, ReminderAgentManagerNapi::Common::getErrCodeMsg(ret));
        return;
    }
    ret = Notification::ReminderHelper::RemoveNotificationSlot(slot);
    if (ret != ERR_OK) {
        ::taihe::set_business_error(ret, ReminderAgentManagerNapi::Common::getErrCodeMsg(ret));
    }
}
...
void AddExcludeDateSync(double reminderId, uintptr_t date)
{
    ...
    ret = Notification::ReminderHelper::AddExcludeDate(id, static_cast<int64_t>(dateValue));
    if (ret != ERR_OK) {
        ::taihe::set_business_error(ret, ReminderAgentManagerNapi::Common::getErrCodeMsg(ret));
    }
}
...
void DeleteExcludeDatesSync(double reminderId)
{
    ...
    int32_t ret = Notification::ReminderHelper::DelExcludeDates(id);
    if (ret != ERR_OK) {
        ::taihe::set_business_error(ret, ReminderAgentManagerNapi::Common::getErrCodeMsg(ret));
    }
}
...
::taihe::array<uintptr_t> GetExcludeDatesSync(double reminderId)
{
    ...
    int32_t ret = Notification::ReminderHelper::GetExcludeDates(id, dates);
    if (ret != ERR_OK) {
        ::taihe::set_business_error(ret, ReminderAgentManagerNapi::Common::getErrCodeMsg(ret));
        return ::taihe::array<uintptr_t>(results);
    }
