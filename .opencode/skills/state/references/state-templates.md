...
**字段说明**:
- `name`：需求/问题名称，用作目录名，由 Architecture 分析后生成或用户指定
- `kb_dir`：完整工作目录路径，由调用方指定
- `requirement`：用户提供的原始需求描述
...
| `status` | string | 任务状态：pending / running / executed / reviewed / done / failed / human_review |
| `retry_count` | integer | 当前重试次数（0-3） |
| `last_error` | string | 最后一次失败的错误信息 |
| `reviewed` | boolean | 是否已通过 Review Skill 检视 |
| `build_verified` | boolean | 是否已通过 Build Skill 编译验证 |
| `build_error_type` | string | 编译失败错误类型：COMPILE_ERROR / LINK_ERROR / DEPENDENCY_MISSING / GN_CONFIG_ERROR |
...
[
  {
    ...
    "summary": "架构设计文档已确认，名称确定为 notification-group-management",
    "document": "{kb_dir}/architecture.md"
  },
  {
    "phase": "dev_design",
    "decision": "approved",
