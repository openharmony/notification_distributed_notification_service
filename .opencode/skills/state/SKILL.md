---
name: state
description: 状态管理 skill。专门管理状态文件 state.json，是状态管理的唯一入口，为调用方提供决策所需的状态数据。与工作流无关，由调用方传入工作目录。
---
# State Skill — 状态管理
...
```json
{
  ...
  "phases": {
    ...
    "build": {"status": "<状态>"},
    "verify": {"status": "<状态>", "skipped": "<bool>", "skip_reason": "<string|null>"},
    "doc": {"status": "<状态>"}
  }
}
```
...
2. 若 `kb_dir` 为null，**不创建目录**，仅在内存中初始化状态数据结构，`kb_dir` 和 `name` 字段留空，等待 Architecture 阶段确定后由调用方调用 `update_info` 补充
3. 若 `kb_dir` 已提供，检查目录是否存在并创建，初始化状态文件内容（详见 @references/state-templates.md），立即写入磁盘
...
**内部流程**：
1. 读取状态文件
2. 遍历 `execute.tasks`，检查每个任务：
   - 若 `status == "pending"` 且 `depends` 中所有任务 `status in ["done", "reviewed"]`，加入 `ready_tasks`
   - 若 `status == "pending"` 且 `depends` 中存在任务 `status in ["failed", "human_review"]`，加入 `blocked_by_failure`
   - 若 `status == "pending"` 且存在依赖任务状态不在 `["done", "reviewed", "failed", "human_review"]` 中，加入 `blocked_tasks`
   - 若 `status == "running"`，加入 `running_tasks`
   - 若 `status == "executed"`，表示代码开发完成等待 Review，不计入 ready/blocked/running
   - 若 `status == "reviewed"`，表示 Review 通过等待 Build，不计入 ready/blocked/running
...
- `kb_dir`
- `action: "update_phase"`
- `phase`: "architecture" | "dev_design" | "plan" | "execute" | "build" | "verify" | "doc"
- `status`: "pending" | "running" | "pending_user_confirmation" | "done" | "blocked" | "skipped"
...
- `action: "update_task"`
- `task_id`
- `status`: "pending" | "running" | "executed" | "reviewed" | "done" | "failed" | "human_review"
- `started_at`（可选）：任务开始时间
- `completed_at`（可选）：任务完成时间
- `reviewed`（可选）：是否通过 Review Skill 检视
- `build_verified`（可选）：是否通过 Build Skill 编译验证
- `build_error_type`（可选）：编译失败错误类型
...
3. 检查每个依赖任务的 `status`
  4. 若所有依赖任务 `status in ["done", "reviewed"]`，返回 `dependency_satisfied: true`
5. 否则返回未完成的依赖列表
...
- `current_phase`: "architecture" | "dev_design" | "plan" | "execute" | "build" | "verify" | "doc" | "completed"
...
3. **唯一入口**：只有你能读取和修改状态文件，其他代理/工作流通过你获取信息
