...
/**
 * Indicates that priority intelligent switch.
 */
static const char* const KEY_PRIORITY_INTELLIGENT_SWITCH = "priorityIntelligentSwitch";

/**
 * Indicates that priority notification V2 switch for bundle.
 */
static const char* const KEY_PRIORITY_NOTIFICATION_SWITCH_FOR_BUNDLE_V2 = "priorityNotificationSwitchForBundleV2";

/**
 * Indicates that priority strategy for bundle.
 */
    ...
    const NotificationPreferencesInfo::BundleInfo &bundleInfo)
{
    ...
    if ((OsAccountManagerHelper::GetInstance().GetOsAccountLocalIdFromUid(
        ...
        ANS_LOGE("Get valid userId failed, function: %{public}s, bundleUid: %{public}d",
            __FUNCTION__, bundleInfo.GetBundleUid());
        NotificationAnalyticsUtil::ReportModifyEvent(message.BranchId(BRANCH_1));
        return false;
    }
    bool result = false;
...
bool NotificationPreferencesDatabase::CheckBundle(const std::string &bundleName, const int32_t &bundleUid)
{
    ...
    if ((OsAccountManagerHelper::GetInstance().GetOsAccountLocalIdFromUid(bundleUid, userId) != ERR_OK) ||
        (userId <= 0)) {
        ANS_LOGE("Get valid userId failed, function: %{public}s, bundleUid: %{public}d", __FUNCTION__, bundleUid);
        return false;
    }
    GetValueFromDisturbeDB(bundleKeyStr, userId, [&](const int32_t &status, std::string &value) {
        switch (status) {
    ...
    const std::string &bundleKey, const int32_t &bundleUid)
{
    ...
    if (OsAccountManagerHelper::GetInstance().GetOsAccountLocalIdFromUid(bundleUid, userId) != ERR_OK ||
        userId <= 0) {
        ANS_LOGE("Failed to get valid userId from uid, function: %{public}s, uid: %{public}d", __FUNCTION__, bundleUid);
        return false;
    }
...
void NotificationPreferencesDatabase::GetDistributedEnabledForClone(const int32_t &userId,
    std::vector<NotificationCloneBundleInfo> &cloneBundles)
{
    for (auto& bundleItem : cloneBundles) {
        NotificationDistributedBundle pcDistributedBundle;
        NotificationDistributedBundle tableDistributedBundle;
        NotificationPreferencesInfo::BundleInfo bundleInfo;
        ...
        collaborationSwitch.tabletLiveView_ = tableDistributedBundle.GetLiveViewEnable();
        collaborationSwitch.tabletNotification_ = tableDistributedBundle.GetNotificationEnable();
    ...
    const std::unordered_map<std::string, std::string> &values, std::vector<NotificationDistributedBundle>& bundles)
{
    ...
    for (auto item : values) {
        ...
        // filter not current user id anco application.
        if (bundleInfo.GetBundleUserId() != -1 && bundleInfo.GetBundleUserId() != userId) {
            ANS_LOGI("Anco userid %{public}s.", bundleKey.c_str());
            continue;
        }
    ...
    NotificationPreferencesInfo::BundleInfo &bundleInfo, const std::string &value) const
{
    ...
    if (level < static_cast<int32_t>(NotificationSlot::NotificationLevel::LEVEL_NONE) ||
        ...
        ANS_LOGE("invalid NotificationLevel: %{public}d, function: %{public}s", level, __FUNCTION__);
        return;
    }
...
bool NotificationPreferencesDatabase::RemoveAnsBundleDbInfo(std::string bundleName, int32_t uid)
{
    ...
    if ((OsAccountManagerHelper::GetInstance().GetOsAccountLocalIdFromUid(uid, userId) != ERR_OK) ||
        (userId <= 0)) {
        ANS_LOGE("Get valid userId failed, function: %{public}s, uid: %{public}d", __FUNCTION__, uid);
        return false;
    }
    int32_t result = rdbDataManager_->DeleteData(key, userId);
    if (result != NativeRdb::E_OK) {
    ...
    const sptr<NotificationBundleOption> &bundleOption, const NotificationConstant::PriorityEnableStatus enableStatus)
{
    ...
    if ((OsAccountManagerHelper::GetInstance().GetOsAccountLocalIdFromUid(
        ...
        ANS_LOGE("Get valid userId failed, function: %{public}s, uid: %{public}d",
            __FUNCTION__, bundleOption->GetUid());
        return false;
    }
...
bool NotificationPreferencesDatabase::PutPriorityIntelligentEnabled(const NotificationConstant::SWITCH_STATE enabled)
{
    ...
    std::string key =
        std::string(KEY_PRIORITY_INTELLIGENT_SWITCH).append(KEY_UNDER_LINE).append(std::to_string(userId));
    int32_t result = PutDataToDB(key, static_cast<int32_t>(enabled), userId);
    if (result != NativeRdb::E_OK) {
        ANS_LOGW("PutPriorityIntelligentEnabled key: %{public}s, enabled: %{public}d, result: %{public}d",
            key.c_str(), enabled, result);
    }
    return (result == NativeRdb::E_OK);
}

bool NotificationPreferencesDatabase::GetPriorityIntelligentEnabled(NotificationConstant::SWITCH_STATE &enabled)
{
    int32_t userId = SUBSCRIBE_USER_INIT;
    OHOS::AccountSA::OsAccountManager::GetForegroundOsAccountLocalId(userId);
    if (userId == SUBSCRIBE_USER_INIT) {
    ...
    if (!result) {
        ANS_LOGW("GetPriorityIntelligentEnabled key: %{public}s, result: %{public}d, enabled: %{public}d",
            key.c_str(), result, static_cast<int32_t>(enabled));
    }
    return result;
}

bool NotificationPreferencesDatabase::PutPriorityEnabledByBundleV2(
    const sptr<NotificationBundleOption> &bundleOption, const NotificationConstant::SWITCH_STATE priorityStatus)
{
    ...
    int32_t result = PutDataToDB(key, static_cast<int32_t>(priorityStatus), userId);
    ANS_LOGI("PutPriorityEnabledByBundleV2 key: %{public}s, enable: %{public}d, result: %{public}d", key.c_str(),
        static_cast<int32_t>(priorityStatus), result);
    return (result == NativeRdb::E_OK);
}

bool NotificationPreferencesDatabase::GetPriorityEnabledByBundleV2(
    const sptr<NotificationBundleOption> &bundleOption, NotificationConstant::SWITCH_STATE &priorityStatus)
{
    ...
    std::string key = GenerateBundleKey((bundleOption->GetBundleName().append(std::to_string(bundleOption->GetUid())) +
        KEY_UNDER_LINE + std::to_string(bundleOption->GetUid())), KEY_PRIORITY_NOTIFICATION_SWITCH_FOR_BUNDLE_V2);
    bool result = false;
    priorityStatus = NotificationConstant::SWITCH_STATE::SYSTEM_DEFAULT_ON;
    GetValueFromDisturbeDB(key, userId, [&](const int32_t &status, std::string &value) {
        switch (status) {
            case NativeRdb::E_EMPTY_VALUES_BUCKET: {
                result = true;
    ...
    if (!result) {
        ANS_LOGW("GetPriorityEnabledByBundleV2 key: %{public}s, result: %{public}d, priorityStatus: %{public}d",
            key.c_str(), result, static_cast<int32_t>(priorityStatus));
    }
    return result;
}

bool NotificationPreferencesDatabase::PutPriorityStrategyByBundle(
    const sptr<NotificationBundleOption> &bundleOption, const int64_t strategy)
{
    ...
    int32_t result = PutDataToDB(key, strategy, userId);
    ANS_LOGI("PutPriorityStrategyByBundle key: %{public}s, strategy: %{public}d, result: %{public}d",
        key.c_str(), static_cast<int32_t>(strategy), result);
    return (result == NativeRdb::E_OK);
}

bool NotificationPreferencesDatabase::GetPriorityStrategyByBundle(
    const sptr<NotificationBundleOption> &bundleOption, int64_t &strategy)
{
    ...
    std::string key = GenerateBundleKey((bundleOption->GetBundleName().append(std::to_string(bundleOption->GetUid())) +
        KEY_UNDER_LINE + std::to_string(bundleOption->GetUid())), KEY_PRIORITY_STRATEGY_FOR_BUNDLE);
    bool result = false;
    strategy = PRIORITY_STRATEGY_DEFAULT;
    GetValueFromDisturbeDB(key, userId, [&](const int32_t &status, std::string &value) {
        switch (status) {
            case NativeRdb::E_EMPTY_VALUES_BUCKET: {
                result = true;
    ...
    if (!result) {
        ANS_LOGW("GetPriorityStrategyByBundle key: %{public}s, result: %{public}d, strategy: %{public}d",
            key.c_str(), result, static_cast<int32_t>(strategy));
    }
    return result;
}
    ...
    const NotificationPreferencesInfo::SilentReminderInfo &silentReminderInfo)
{
    ...
    if (OsAccountManagerHelper::GetInstance().GetOsAccountLocalIdFromUid(silentReminderInfo.uid, userId) != ERR_OK ||
        userId <= 0) {
        ANS_LOGE("Failed to get valid userId from uid, function: %{public}s, uid: %{public}d",
            __FUNCTION__, silentReminderInfo.uid);
        return false;
    }
