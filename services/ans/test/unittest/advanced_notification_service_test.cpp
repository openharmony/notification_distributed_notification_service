...
HWTEST_F(AdvancedNotificationServiceTest, IsLiveViewCanRecoverTest_0004, Function | SmallTest | Level1)
{
    ...
    auto now = std::chrono::system_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch());
    request->SetMaxFinishTime(duration.count() + NotificationConstant::MAX_FINISH_TIME);
    request->SetMaxUpdateTime(duration.count() + NotificationConstant::MAX_UPDATE_TIME);
