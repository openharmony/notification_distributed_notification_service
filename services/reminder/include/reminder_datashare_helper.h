...
public:
     ...
     */
    bool Query(std::map<int32_t, sptr<ReminderRequest>>& reminders);
...
private:
     ...
     */
    std::map<int32_t, sptr<ReminderRequest>> CreateReminder(
...
private:
    ...
    std::atomic<bool> deleteTask_ {false};
    std::atomic<int64_t> insertTime_ {0};
    std::atomic<int64_t> deleteTime_ {0};
