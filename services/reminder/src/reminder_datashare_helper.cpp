...
bool ReminderDataShareHelper::Query(std::map<int32_t, sptr<ReminderRequest>>& reminders)
{
    auto helper = CreateDataShareHelper();
    if (helper == nullptr) {
        ANSR_LOGE("Create datashare helper failed.");
...
{
    std::map<int32_t, sptr<ReminderRequest>> reminders = CreateReminder(info);
    ...
    const std::shared_ptr<DataShare::DataShareResultSet>& result)
{
    ...
    std::string title;
    GetRdbValue<std::string>(result, ReminderCalendarShareTable::TITLE, title);
    reminder->SetTitle(title);
    std::string content;
    GetRdbValue<std::string>(result, ReminderCalendarShareTable::CONTENT, content);
    reminder->SetContent(content);
    std::string actionButtons;
    GetRdbValue<std::string>(result, ReminderCalendarShareTable::BUTTONS, actionButtons);
    reminder->DeserializeButtonInfoFromJson(actionButtons);
    std::string wantAgent;
    GetRdbValue<std::string>(result, ReminderCalendarShareTable::WANT_AGENT, wantAgent);
...
std::map<int32_t, sptr<ReminderRequest>> ReminderDataShareHelper::CreateReminder(
