...
enum OperationType {
    ...
    DISTRIBUTE_OPERATION_JUMP_BY_TYPE
};
...
public:
    ...
    int32_t GetJumpType() const;

    void SetJumpType(const int32_t jumpType);

    std::string GetNotificationUdid() const;

    void SetNotificationUdid(const std::string& udid);
